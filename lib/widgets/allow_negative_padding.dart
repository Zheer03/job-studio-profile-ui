...
class RenderAllowNegativePadding extends RenderShiftedBox {
  ...
  void _resolve() {
    if (_resolvedPadding != EdgeInsets.zero) return;
  ...
  void performLayout() {
    ...
    size = constraints.constrain(Size(
      _resolvedPadding.left + (child?.size.width ?? 0) + _resolvedPadding.right,
      _resolvedPadding.top + (child?.size.height ?? 0) + _resolvedPadding.bottom,
    ));
  }
