...
class CountryCodeHelper {
  static List<CountryCodeModel> getCountryCodes({String search = ''}) {
    return _countryCode
        .map((e) => CountryCodeModel.fromJson(e))
        .where(
          (element) =>
              element.name.toLowerCase().contains(search.toLowerCase()) ||
              element.dialCode.toLowerCase().contains(search.toLowerCase()) ||
              element.code.toLowerCase().contains(search.toLowerCase()) ||
              element.flag.toLowerCase().contains(search.toLowerCase()),
        )
        .toList();
  }

  static CountryCodeModel getCountryCode(String dialCode) {
    final dc = dialCode.replaceAll('00', '+');
    return getCountryCodes().firstWhere((element) => element.dialCode == dc);
  }

  static CountryCodeModel defaultCountryCode() {
